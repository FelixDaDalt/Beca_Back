...
async function verificarVencimientos() {
    try {
        ...
        await beca_solicitud.update(
            { id_estado: 4, id_resolucion: 3, sinLeerSolicitante: 1, sinLeer: 1 },
            {
                where: {
        ...
        for (const beca of becasVencidas) {
            ...
            if (colegioSolicitante?.email) {
                try {
                    await enviarCorreo(
                        colegioSolicitante.email,
                        "📌 Beca Vencida",
                        `La beca que solicitaste ha vencido.`,
                        `<h1>Beca Vencida</h1><p>La beca solicitada a ${colegioOfrecio.nombre} ha vencido.</p>`
                    );
                } catch (error) {
            ...
            if (colegioOfrecio?.email) {
                try {
                    await enviarCorreo(
                        colegioOfrecio.email,
                        "📌 Beca Vencida",
                        `Una beca que te solicitaron ha vencido.`,
                        `<h1>Beca Vencida</h1><p>Una beca solicitada por ${colegioSolicitante.nombre} ha vencido.</p>`
                    );
                } catch (error) {
            ...
            if (colegioSolicitante?.email) {
                try {
                    await enviarCorreo(
                        colegioSolicitante.email,
                        "⏰ Beca por Vencer",
                        `La beca que solicitaste está por vencer.`,
                        `<h1>Beca por Vencer</h1><p>La beca solicitada a ${colegioOfrecio.nombre} está por vencer.</p>`
                    );
                } catch (error) {
                    console.error(`⚠️ Error al enviar correo a ${colegioSolicitante.email}:`, error);
                }
            ...
            if (colegioOfrecio?.email) {
                try {
                    await enviarCorreo(
                        colegioOfrecio.email,
                        "⏰ Beca por Vencer",
                        `Una beca que te solicitaron está por vencer.`,
                        `<h1>Beca por Vencer</h1><p>Una beca solicitada por ${colegioSolicitante.nombre} está por vencer.</p>`
                    );
                } catch (error) {
                    console.error(`⚠️ Error al enviar correo a ${colegioOfrecio.email}:`, error);
                }
