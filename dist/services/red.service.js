...
exports.meRed = exports.obtenerMiembros = exports.editarMiembrosRed = exports.editarDatosRed = exports.borrarMiembro = exports.colegiosDisponibles = exports.obtenerRed = exports.borrarRed = exports.listadoRedes = exports.altaRed = void 0;
...
const altaRed = async (altaRed, transaction) => {
    try {
        ...
        const colegioInexistente = await colegio_1.colegio.findOne({
            where: {
                id: {
                    [sequelize_1.Op.in]: altaRed.colegios.map((colegio) => colegio.id) // Filtra por lista de IDs
                },
        ...
        const redCreada = await red_1.red.create(altaRed.red, { transaction });
        ...
        try {
            await red_colegio_1.red_colegio.bulkCreate(redColegiosData, { transaction });
        }
        catch (error) {
...
exports.listadoRedes = listadoRedes;
const borrarRed = async (idRed, transaction) => {
    try {
        const redExistente = await red_1.red.findOne({
            where: {
                id: idRed,
                borrado: 0
            }
...
const obtenerRed = async (idRed) => {
    try {
        const redEncontrada = await red_1.red.findOne({
            where: {
                id: idRed,
                borrado: 0
            },
...
const colegiosDisponibles = async (idRed) => {
    try {
        ...
        const miembros = redEncontrada.red_colegios.map((redColegio) => {
        ...
        const idsMiembros = miembros.map((colegio) => colegio.id);
        // Obtener los colegios que NO están en la red
        const disponibles = await colegio_1.colegio.findAll({
            where: {
                id: {
                    [sequelize_1.Op.notIn]: idsMiembros, // Excluir los IDs de los colegios en la red
                },
                borrado: 0, // Asegurarnos de no incluir colegios "borrados"
            },
        });
        // Convertir logos a Base64 en los colegios disponibles
        const disponiblesConLogo = disponibles.map((colegio) => {
            if (colegio.logo) {
...
const borrarMiembro = async (idRed, idColegio, transaction) => {
    try {
        // Obtener la red y los colegios asociados a ella
        const colegioEncontrado = await red_colegio_1.red_colegio.findOne({
            where: {
...
const editarDatosRed = async (datosRed, idRol, idColegio, transaction) => {
    try {
        // Obtener la red y los colegios asociados a ella
        const redEncontrada = await red_1.red.findOne({
            where: {
        ...
        await red_1.red.update(datosRed, {
...
const editarMiembrosRed = async (miembros, idRol, idColegio, transaction) => {
    try {
        // Obtener la red y los colegios asociados a ella
        const redEncontrada = await red_1.red.findOne({
            where: {
        ...
        const colegiosExistentes = await red_colegio_1.red_colegio.findAll({
            where: {
                id_red: miembros.id_red,
            },
        });
        // Crear mapas para identificar rápidamente qué hacer con cada integrante
        const colegiosMap = new Map(colegiosExistentes.map((colegio) => [colegio.id_colegio, colegio]));
        ...
        if (colegiosAActualizar.length > 0) {
            await red_colegio_1.red_colegio.update({ borrado: 0 }, {
                where: {
                    id_red: miembros.id_red,
        ...
        if (colegiosANuevos.length > 0) {
            await red_colegio_1.red_colegio.bulkCreate(colegiosANuevos, { transaction });
        }
        return {
            red: redEncontrada,
            colegiosAActualizar,
...
// };
//Miembros
const obtenerMiembros = async (idRed) => {
    try {
        const miembrosEcnontrados = await red_colegio_1.red_colegio.findAll({
            ...
            order: [
                ...
                [{ model: colegio_1.colegio, as: 'id_colegio_colegio' }, 'nombre', 'ASC'] // Luego ordena por nombre del colegio
            ]
        });
...
const meRed = async (idRed, idColegio) => {
    try {
        const misDatos = await red_colegio_1.red_colegio.findOne({
            where: {
                id_red: idRed,
                id_colegio: idColegio,
                borrado: 0
